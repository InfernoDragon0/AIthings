...
bool parse_args(int argc, char** argv, std::string& wts, std::string& engine, bool& is_p6, float& gd, float& gw, std::string& img_dir) {
    ...
    } else if (std::string(argv[1]) == "-i" && argc == 3) {
        engine = std::string(argv[2]);
    } else {
        return false;
    }
...
std::string TEMP_FILE_NAME = "temp.jpg"; //fix me to be dynamic below

cv::Mat read_temp_file() {
...
int main(int argc, char** argv) {
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir)) {
        ...
        std::cerr << "./yolov5 -i [.engine] // start engine and wait for stdin signals" << std::endl;
        return -1;
    }
    ...
    do {
        getline(std::cin, cmd);
        if (cmd.compare("process_img")) continue; //modify here to accept another argument which is the name of the image to process
        ...
        for (int b = 0; b < fcount; b++) {
            cv::Mat img = read_temp_file();
            if (img.empty()) {
                std::cout << "IMAGE_UNREADABLE" << std::endl;
                image_found = false;
                continue;
            }
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            // print array of detection boxes to stdout
            std::cout << "{\"time_taken_ms\":";
            std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            std::cout << ",\"results\":[";
            for (size_t j = 0; j < res.size(); j++) {
                // json format of bbox
                std::string bbox_json = "";
                for (float i: res[j].bbox) {
                    bbox_json += std::to_string(i) + ",";
                }
                if (!bbox_json.empty()) {
                    bbox_json.pop_back();
                }

                // print detection box in JSON format
                std::cout << "{\"bbox\":[" + bbox_json + "],";
                std::cout << "\"class_id\":" + std::to_string((int)res[j].class_id) + ",";
                std::cout << "\"conf\":" + std::to_string(res[j].conf) + "}";
                if (j < res.size() - 1) {
                    std::cout << ",";
                }
            }
